...
OUTPUT_DIR = PROCESSED_DIR / "models" / "tuned"
...
def _load_csv(path: Path) -> pd.DataFrame:
	if not path.exists():
		raise TuneError(f"Dataset file not found: {path}")
...
def _ensure_columns(df: pd.DataFrame, required: Iterable[str]) -> None:
	...
	if missing:
		raise TuneError(f"Missing required columns: {missing}. Available columns: {list(df.columns)}")
...
def _resolve_key_columns(df: pd.DataFrame) -> List[str]:
	...
	if len(columns) < 2:
		raise TuneError("Insufficient columns to match splits with preprocessed data.")
	return columns
...
def _resolve_row_ids(split_df: pd.DataFrame, base_df: pd.DataFrame) -> List[int]:
	...
	if merged["__row_id"].isna().any():
		...
		raise TuneError(f"{missing} rows could not be matched with preprocessed dataset.")
...
def _resolve_labels(df: pd.DataFrame) -> pd.Series:
	...
	raise TuneError("Label columns not found in dataset.")
...
def _load_features(path: Path, *, sparse_matrix: bool) -> np.ndarray | sparse.spmatrix:
	if not path.exists():
		raise TuneError(f"Feature file not found: {path}")
	if sparse_matrix:
...
def _run_grid_search(features, labels: pd.Series) -> GridSearchCV:
	model = LinearSVC(
...
def main() -> int:
	...
	try:
		...
		if base_df.empty:
			raise TuneError("Preprocessed dataset is empty.")
		...
		if not tfidf_path.exists():
			raise TuneError("TF-IDF features not found. Run feature_extraction before tuning.")
		...
		search = _run_grid_search(X_train, y_train)
		...
		joblib.dump(model, OUTPUT_DIR / "tuned_model.joblib")
		_save_json(OUTPUT_DIR / "tune_metadata.json", metadata)

	except (TuneError, FileNotFoundError, ValueError) as exc:
		print(f"[tune] Error: {exc}")
		return 1

	print(f"[tune] Completed. Output saved to: {OUTPUT_DIR}")
	print(f"[tune] Best feature: {best_feature}")
	print(f"[tune] Validation F1: {best_score:.4f}")
	return 0
